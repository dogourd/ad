...
public class UnitDistrictIndex implements IIndexAware<String, Set<Long>> {
    ...
    public boolean match(Long unitId, List<String> districts) {
        if (redisTemplate.hasKey(UNIT_DISTRICT_INDEX_PREFIX + unitId)) {
            Set<Object> unitDistrictSet = redisTemplate.opsForSet().members(UNIT_DISTRICT_INDEX_PREFIX + unitId);
            if (CollectionUtils.isNotEmpty(unitDistrictSet)) {
                return CollectionUtils.isSubCollection(districts, unitDistrictSet);
            }
        }
        return false;
    }
